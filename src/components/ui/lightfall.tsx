...
const prepColors = (input?: string[]) => {
  ...
  const arr = Array.from({ length: MAX_COLORS }, (_, i) => hexToRGB(base[Math.min(i, base.length - 1)]));
  const avg = [0, 0, 0];
...
vec3 palette(float h) {
  int idx = int(floor(clamp(h, 0.0, 0.999999) * max(float(uColorCount), 1.0)));
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (!paused) renderer.render({ scene });
    };
    rafRef.current = requestAnimationFrame(loop);
