...
export function CoverflowCarousel({
  ...
  falloff = 0.56,
  fade = 0.12,
  cardWidth = "clamp(148px, 28vw, 260px)",
  gap = 0.08,
  loop = true,
  showCaption = false,
  showPagination = false,
  showNavigation = false,
  label = "Lume preview carousel",
  className,
  cardClassName,
  ...
  const goTo = React.useCallback(
    (index: number) => settle(clamp(loop ? index + Math.round((targetRef.current - index) / count) * count : index)),
  ...
  return (
    <div className={cn("w-full", className)} style={{ ["--cf-card" as string]: cardWidth }} role="region" aria-roledescription="carousel" aria-label={label}>
      <div className="relative">
        ...
        {showNavigation && (
          <>
            <button type="button" aria-label="Previous slide" onClick={() => nudge(-1)} className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/70 p-2 text-slate-950 backdrop-blur transition hover:bg-white">
              <ChevronLeft className="size-5" />
            </button>
            <button type="button" aria-label="Next slide" onClick={() => nudge(1)} className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/70 p-2 text-slate-950 backdrop-blur transition hover:bg-white">
