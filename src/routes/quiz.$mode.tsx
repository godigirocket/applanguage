...
function QuizPage() {
  ...
  const totalQ =
    effectiveMode === "streak"
      ? currentIdx
      : Math.min(questions.length, effectiveMode === "daily" ? 15 : 10);

  // Early return for flashcards mode (after all hooks)
  if (mode === "flashcards") {
    return (
      <div
        style={{
    ...
    targetLanguage === "en"
      ...
      : targetLanguage === "es"
        ? [
            ...
            "Disculpe, ¿dónde está la estación más cercana?",
            "Es un gran placer conocerte esta tarde.",
            "¿Podrías hablar un poco más despacio, por favor?",
          ]
        : [
            "Olá, tudo bem com você?",
            "Gostaria de pedir um café bem quente, por favor.",
            "Com licença, onde fica a praia mais próxima?",
            "É um grande prazer conhecer você hoje.",
            "Você poderia falar um pouco mais devagar?",
          ];

  if (mode === "pronunciation" || mode === "speak") {
    const isCompleted = voiceStep >= voicePhrases.length;
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            >
              <div
                style={{
                  height: "100%",
                  background: "var(--brand)",
                  width: `${(voiceStep / voicePhrases.length) * 100}%`,
            ...
            <span style={{ fontSize: "13px", fontWeight: 800, color: "var(--text-secondary)" }}>
              {voiceStep + 1}/{voicePhrases.length}
            </span>
          </div>
  ...
  const nextQuestion = () => {
    const limit = effectiveMode === "daily" ? 14 : 9;
    ...
    >
      ...
      >
        ...
        <div
          style={{
            ...
            padding: "6px 12px",
            borderRadius: "99px",
            background: qStreak >= 3 ? "rgba(255,107,53,0.1)" : "var(--surface)",
            border: qStreak >= 3 ? "1px solid rgba(255,107,53,0.3)" : "1px solid var(--border)",
          }}
        >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    state === "correct"
                      ? "0 4px 16px rgba(74,122,90,0.2)"
                      : state === "wrong"
                        ? "0 4px 16px rgba(196,113,74,0.2)"
      ...
      {isAnswered && (
        ...
        >
          ...
          >
            {effectiveMode === "streak" && selected !== q.correct
              ? "🏆 See Results"
              : currentIdx >= totalQ - 1 && effectiveMode !== "streak"
                ? "🏆 See Results"
                : "Continue →"}
          </button>
        </div>
...
function QuizResults({
  ...
  const earnedXP =
    mode === "streak"
      ? score * 10
      : Math.round((score / Math.max(total, 1)) * (mode === "daily" ? 200 : 100));
