...
import {
  getLessonCatalogue,
  type Lesson as EngineLesson,
  type LessonStep as EngineLessonStep,
  type TargetLanguage as EngineLanguage,
} from "./lessonEngine";
...
function engineLessonToCard(
  engineLesson: EngineLesson,
  index: number,
...
function manualLessonToCard(
  manualLesson: (typeof ALL_LESSONS)[0],
  completedLessons: string[],
): LessonCard {
...
function withProgressState(catalog: LessonCard[], completedLessons: string[]): LessonCard[] {
  return catalog.map((lesson) => {
    const completed = completedLessons.includes(lesson.id);
    return {
      ...lesson,
      completed,
      progress: completed ? 100 : lesson.progress,
      locked: lesson.isPremium ? false : checkIfLocked(lesson.id, completedLessons, catalog),
    };
  });
}
...
export function generateLessonCatalog(completedLessons: string[] = []): LessonCard[] {
  ...
  if (cachedCatalog && cachedCatalog.length > 0) {
    return withProgressState(cachedCatalog, completedLessons);
  }
  ...
  return withProgressState(catalog, completedLessons);
}
...
function checkIfLocked(
  ...
  if (!lesson || lesson.isPremium) return false;

  const languageLessons = allLessons.filter((l) => !l.isPremium && l.language === lesson.language);
  const index = languageLessons.findIndex((l) => l.id === lessonId);
  if (index <= 0) return false; // First lesson always unlocked

  // Check if previous lesson is completed
  const prevLesson = languageLessons[index - 1];
